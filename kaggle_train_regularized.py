...
def train_model(config, args):
    """Train emotion recognition model with regularization."""
    # Setup logger
    logger = setup_logger(config['log_dir'], 'training')
    logger.info("=" * 60)
    logger.info("Starting Training with Regularization")
    logger.info("=" * 60)
    ...
    logger.info(f"Gradient clipping enabled: {args.grad_clip}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train emotion recognition model on Kaggle - Regularized Approach')
    ...
    parser.add_argument('--lr', type=float, default=0.0005, 
                       help='Learning rate (default: 0.0005, reduced from 0.001)')
    ...
    parser.add_argument('--weight_decay', type=float, default=1e-3,
                       help='Weight decay (default: 1e-3, increased from 1e-4)')
    ...
    # Print configuration
    print("=" * 60)
    print("Kaggle Training Configuration (Regularized)")
    print("=" * 60)
